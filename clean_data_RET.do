...
* Creation of repeated decision var

by  idcorto , sort: generate dec_num =_n
move   dec_num  return_0
...
gen return =  return_0
replace  return =  return_5 if  dec_num==2
replace  return =  return_10 if  dec_num==3
replace  return =  return_15 if  dec_num==4
replace  return =  return_20 if  dec_num==5
replace  return =  return_25 if  dec_num==6

move return return_0

gen transfer_sm = 0
replace transfer_sm = 5 if  dec_num==2
replace transfer_sm = 10 if  dec_num==3
replace transfer_sm = 15 if  dec_num==4
replace transfer_sm = 20 if  dec_num==5
replace transfer_sm = 25 if  dec_num==6

gen return_share =  return/(2*transfer_sm+25)
gen transfer_sm_2 = transfer_sm^2

gen return_max=2*transfer_sm+25
gen return_max_perc = return_max/75
...
* Normalizzazione exp over receivers 
...
* costruisco una variabile che 蠽1 se uno 衭embro oggi o lo 衳tato in passato: MEMBROALMENO (identico a membroalmeno, ora tolto; 5-4-12 eliminato membroalmeno completamente, era ancora presente in alcune def.).
gen membroalmeno = membroatt==1 | membropass==1
replace membroalmeno=. if membroatt==. & membropass==.

*costruisco variabili dummy per chi 衭embro solo attualmente, lo 衳tato solo in passato, lo 衳ia ora sia lo 衳tato in passato: MEMBROSOLOORA
...
* ossia devo avere 3 gruppi di soggetti: quelli che prendono valori diversi da 0 nell'interazione con ingroup devono essere tutti volontari, quelli che prendono diverso da 0 nell'outgroup anche, cos졬a variabile residuale sono i non membri 
