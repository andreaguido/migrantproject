...
legend(order(1 "North" 2 "South")) bar(1, fcolor(gray)) bar(2, fcolor(black)) ytitle(FE on Amount Retuned)
...
legend(order(1 "South" 2 "North")) ytitle(% of Obs.)
...
graph bar return_south return_north , over(AmountSent ) legend(order(1 "South" 2 "North")) title("Amount Returned") ylabel(0(5)30) ///
